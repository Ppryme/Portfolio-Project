...
export default function Sidebar ({open, setOpen,hireMePopup,setIsHireMePopup,darkMode}) {
  ...
  const portalTarget = document.getElementById("sidebar")
    ...
    return createPortal (
         <div className = {`${darkMode && " dark:bg-[#102D44] dark:text-white  "}flex z-2  flex-col space-y-4 gap-5 pl-3 pt-12 items-start bg-white fixed h-full w-72 top-0 right-0 text-left shadow-xl
      ...
      <nav className= "hover:none flex flex-col gap-5 space-y-4  text-lg font-medium ">
        ...
        <Link to="/" className={` ${darkMode && "hover:text-white"} nav-link`}    href="">Home</Link>
        <Link to= "/project-page"  className={` ${darkMode && "hover:text-white"} nav-link`}href="">Projects</Link>
        <Link to= "/Aboutme" className={` ${darkMode && "hover:text-white"} nav-link`}href="">About me</Link>
        <Link to="contact-page" className={` ${darkMode && "hover:text-white"} nav-link`}href="">Contact</Link>
